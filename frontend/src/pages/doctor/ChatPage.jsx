    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3">
        {!chatId ? (
          ...
          messages.map((msg, i) => {
            // is_mine определяем по sender_id если есть user в контексте,
            // иначе по sender_name (врач видит своё имя)
            const isMine = user?.id
              ? msg.sender_id === user.id
              : msg.sender_name === user?.login || msg.sender_name === user?.name
            return (
