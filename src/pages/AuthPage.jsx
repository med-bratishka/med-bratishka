...
export default function AuthPage() {
  ...
  const handleSubmit = () => {
    const mockUser = {
      id: 1,
      name: role === "doctor" ? "Dr. Ivanov" : "Alena",
      role,
      token: "mock-token",
    }
    login(mockUser)
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="flex items-center gap-2 mb-6 justify-center">
          ...
          <span className="text-xl font-medium text-gray-800">MedCare</span>
        </div>
          ...
          <div className="p-7 flex flex-col gap-4">
            {mode === "register" && (
              <div className="flex flex-col gap-1.5">
                ...
                <input className="input-field" name="name" placeholder="Ivan Ivanov" value={form.name} onChange={handleChange} />
              </div>
            )}
            <div className="flex flex-col gap-1.5">
            ...
            <button onClick={handleSubmit} disabled={loading} className="btn-primary w-full py-2.5 mt-1 disabled:opacity-50">
              {loading ? "Loading..." : mode === "login" ? `Login as ${isPatient ? "patient" : "doctor"}` : "Register"}
            </button>
            <div className="flex items-center gap-3">
              <div className="flex-1 h-px bg-gray-100" />
              <span className="text-xs text-gray-400">{mode === "login" ? "no account?" : "have account?"}</span>
              <div className="flex-1 h-px bg-gray-100" />
            </div>
